        ...
        if filename.endswith(".mxl"):
                        ...
                        if "media-type" in rootfile_tag.attrib:
                            if rootfile_tag.attrib["media-type"] == MUSICXML_MIME_TYPE:
                                if not compressed_file_name:
                                    compressed_file_name = rootfile_tag.attrib[
                                        "full-path"
                                    ]
                                else:
            ...
            try:
                compressed_file_info = [
                    x for x in infolist if x.filename == compressed_file_name
                ][0]
            except IndexError:
                raise MusicXMLParseError(
...
class ScorePart(object):
    ...
    def _parse(self, xml_score_part):
        ...
        if (
            xml_midi_instrument is not None
            and xml_midi_instrument.find("midi-channel") is not None
            and xml_midi_instrument.find("midi-program") is not None
        ):
    ...
    def _parse(self):
        ...
        for child in self.xml_note:
            if child.tag == "chord":
                ...
                self.note_duration.parse_duration(
                    self.is_in_chord, self.is_grace_note, child.text
                )
...
class ChordSymbol(object):
            ...
            if child.tag == "root":
                ...
                try:
                    ...
                    raise ChordSymbolParseError(
                        "Non-integer offset: " + str(child.text)
                    )
    ...
    def _parse_root(self, xml_root):
        ...
        self.root = self._parse_pitch(
            xml_root, step_tag="root-step", alter_tag="root-alter"
        )

    def _parse_bass(self, xml_bass):
        """Parse the <bass> tag for a chord symbol."""
        self.bass = self._parse_pitch(
            xml_bass, step_tag="bass-step", alter_tag="bass-alter"
        )
    ...
    def _parse_degree(self, xml_degree):
        ...
        try:
            ...
            raise ChordSymbolParseError("Non-integer scale degree: " + str(value_text))
...
class TimeSignature(object):
    ...
    def _parse(self):
        ...
        if (
            len(self.xml_time.findall("beats")) > 1
            or len(self.xml_time.findall("beat-type")) > 1
        ):
...
class KeySignature(object):
    ...
    def _parse(self):
        ...
        if fifths is None:
            raise KeyParseError("Could not find fifths attribute in key signature.")
